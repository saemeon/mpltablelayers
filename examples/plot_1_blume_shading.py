"""# Shading, censoring, header spans, and annotations on a blume table

Four overlay layers using ``add_table_multispan_cell`` and
``add_table_cell_annotation``.

blume's ``Table`` exposes the same ``get_celld()`` / ``get_figure()`` interface
as ``matplotlib.table.Table``, so the approach is identical:
...
table = blume_table(ax, cellText=cell_text, loc="center", edges="B")
...
censor = add_table_multispan_cell(
    table, y0x0=(7, 1), width=3, height=2,
    facecolor="black", zorder=3, text="REDACTED", loc="center",
)
...
# blume Cell has no get_text(); SpanCell at zorder=3 covers underlying text anyway
for c in range(5):
...
title = add_table_multispan_cell(
    table, y0x0=(0, 0), width=5, height=1,
    facecolor="#1a5276", zorder=3, text="Student Performance", loc="center",
)
...
scores_hdr = add_table_multispan_cell(
    table, y0x0=(1, 1), width=3, height=1,
    facecolor="#2980b9", zorder=3, text="Scores", loc="center",
)
