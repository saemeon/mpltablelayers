...
add_plottable_multispan_cell(
    tab, row=0, col=0, width=5, height=4, facecolor="#d4e6f1"
)
...
ax.text(
    ...
    "REDACTED",
    color="white", rotation=30, ha="center", va="center",
...
ax.text(
    2.5, -1.5, "Student Performance",
    color="white", ha="center", va="center", fontsize=11, weight="bold", zorder=4,
)

# ── Layer 3b: "Scores" sub-header (col_label_row, cols 1–3) ─────────────────
add_plottable_multispan_cell(
    tab, row=-1, col=1, width=3, height=1, facecolor="#2980b9"
)
...
ax.text(
    ...
    "Scores",
    color="white", ha="center", va="center", fontsize=9, weight="bold", zorder=4,
)

# ── Layer 4: ellipse annotation calling out Dave's Math score (95) ───────────
# Data rows 0-indexed: Dave = row 3, Math = col 1
ann = add_plottable_cell_annotation(tab, row=3, col=1, edgecolor="#c0392b", linewidth=2.5, padding=0)
ax.annotate(
